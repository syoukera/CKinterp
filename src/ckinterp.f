...
C     3.  Write LENICK, LENRCK, LENCCK to linking file
...
C     1.  First record of linking file now consists of a character
...
C     (Unit16), and binary linking data is written to LINC (Unit25).
C
...
C
      PARAMETER (MDIM=50, KDIM=500, MKDIM=MDIM*KDIM, IDIM=1100, LSYM=16,
     1           NPAR=3, NPIDIM=IDIM*NPAR, NPC=5, NPCP2=NPC+2, MAXTP=3,
     2           NTR=MAXTP-1, NKTDIM=NTR*NPCP2*KDIM, MAXSP=8, MAXTB=10,
     3           NLAR=2, NSIDIM=MAXSP*IDIM, NTIDIM=MAXTB*IDIM,
     4           NLIDIM=NLAR*IDIM, NFAR=8, NFIDIM=NFAR*IDIM,
     5           NTDIM=KDIM*MAXTP, NIDIM=9*IDIM, LIN=15, LOUT=16,
     6           LTHRM=17, LINC=25)
C
...
C
     ...
     1     ITASK,NCHRG,MM,KK,II,NLAN,NFAL,NTHB,NREV,NRLT,NWL/11*0/,
     ...
     6     NSPEC,NREAC,IREV,ILAN,IRLT,IWL,IFAL,KFAL,ITHB/NIDIM*0/
C
...
C            OPEN (LTHRM, STATUS='OLD', FORM='FORMATTED', READONLY,
C     1            SHARED)
...
C
     ...
     2                   NFAL, IFAL, IDUP, NFAR, PFAL, IFOP, KFAL, NLAN,
     3                   ILAN, NLAR, PLAN, NREV, IREV, RPAR,
     4                   NRLT, IRLT, RLAN, NWL, IWL, WL, KERR)
C
         ELSE
C
...
C
     ...
     5                      NLAN, ILAN, NRLT, IRLT, KERR, LOUT)
C
C              NEW REACTION
C
               II = II+1
...
C----------------------------------------------------------------------C
     ...
     2                   NFAL, IFAL, KFAL, NWL, IWL, WL, KERR)
C
...
C
     ...
     2          IPLUS(20)
      CHARACTER KNAME(*)*(*), LINE*(*), INUM(10)*1, UPCASE*4
      CHARACTER*80 ISTR, IREAC, IPROD, ISPEC, INAME, ITEMP
      LOGICAL KERR, LTHB, LWL
...
C
            ...
            ENDIF
C
C-----------does this string start with a one- or two-digit number?
C
            IVAL = 0
            CALL CKCOMP (ISPEC(1:1), INUM, 10, I1)
            CALL CKCOMP (ISPEC(2:2), INUM, 10, I2)
            IF (I1 .GT. 0) THEN
               ITEMP = ' '
               IF (I2 .GT. 0) THEN
                  IVAL = 10*(I1-1) + (I2-1)
...
C
     ...
     1          AIK(MAXTB,*), IFAL(*), KFAL(*), IFOP(*), PFAL(NFAR,*),
     2          ILAN(*), PLAN(NLAR,*), IREV(*), RPAR(NPAR,*), IRLT(*),
     3          RLAN(NLAR,*), IWL(*), WL(*), VAL(1)
...
C
 ...
 1060 FORMAT (6X,'Error...reaction does not balance...')
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKBAL (NSPEC, KSPEC, KCOEF, MDIM, MM, KCHRG, KNCF,
     1                  IERR)
C
C     Checks elemental balance of reactants vs. products.
C     Checks charge balance of reaction.
C
C     Input:  NSPEC - total number of species in this reaction
C             KSPEC(N),N=1,NSPEC- array of species numbers in reaction
      ...
      DO 85 N = 1, ILASCH(LINE)-3
         ...
         IF (AUNITS .EQ. ' ') THEN
            IF (UPCASE(LINE(N:), 4) .EQ. 'MOLE') THEN
               IF (UPCASE(LINE(N+4:), 1) .EQ. 'S') THEN
                  AUNITS = 'MOLE'
                  IF (IUNITS .EQ. ' ') THEN
                  ...
                  ENDIF
               ELSEIF (UPCASE(LINE(N+4:), 4) .EQ. 'CULE') THEN
                  AUNITS = 'MOLC'
                  IF (IUNITS .EQ. ' ') THEN
